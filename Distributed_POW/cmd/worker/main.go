...
func main() {
	...
	for res := range worker.ResultChannel {
		result := distpow.CoordResultArgs{
			...
			Secret:       	  res.Secret,
			WorkerTraceToken: res.WorkerTrace.GenerateToken(),
		}
		var reply distpow.WorkerCoordReply
		call := worker.Coordinator.Go("CoordRPCHandler.Result", result, &reply, nil)
		<- call.Done
		// worker.Tracer.ReceiveToken(reply.CoordTraceToken)
	}
}
